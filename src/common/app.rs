...
use crate::common::shared::{shared_config, REPLICATION_INTERVAL};
    ...
    pub fn new(settings: Settings, cli: Cli, name: String) -> Self {
        match cli.mode {
            ...
            Some(Mode::Separate {
                ...
                let (mut client_app, client_config) = client_app(
                    settings.clone(),
                    net_config,
                );
            ...
            Some(Mode::Client {
                ...
                let net_config = get_client_net_config(
                    &settings, client_id,
                );
                let (mut app, config) = client_app(
                    settings, net_config,
                );
                app.add_plugins(ExampleClientRendererPlugin::new(name));
                Apps::Client {
